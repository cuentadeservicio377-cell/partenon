...
The repository is at `https://github.com/cuentadeservicio377-cell/partenon`. For now, open an issue with a reproducible command, expected output, and actual output. There is no CI yet, so include the result of `python scripts/demo_tesorero.py` and `cd dashboard && npm run build`.
