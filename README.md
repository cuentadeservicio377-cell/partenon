...
1. **`web/index.html`** — Página de marketing con arco narrativo de 4 pasos (intención → héroes → misiones → entrega), before/after, caso concreto (un día con Hermes en Cafetería Aurora), stack del hackathon (Hermes + NVIDIA NemoClaw/OpenShell + Nemotron 3 Ultra + Stripe Skills), héroes en tabs/acordeón, G-Brain, auto-mejora, messaging gateway, impacto con contadores animados y milestone bar 10 → 1M, métricas de diseño, carta de intención, growth plan de 4 canales, CTA tipeado e instalación.
2. **`web/developers.html`** — Página técnica. Arquitectura de agentes con NVIDIA NemoClaw/OpenShell (Hermes harness), Eval Loop, diagramas Mermaid flat, stack, stack del hackathon, perfiles técnicos de los 6 héroes con badges y tablas de especificación, diagramas de conexión por perfil, G-Brain/MCP con API reference, Hermes Core (skills, memory, cron, subagents, gateway, MCP, self-improvement), demo funcional del Tesorero, workshop de 90 min con timeline visual, install tabs (Local / NemoClaw / Stripe / Variables) con feedback de copiado, estructura de repositorio y roadmap.
...
- Web verificada: `web/index.html` y `web/developers.html` en desktop (1440px) y mobile (390px). Screenshots actualizadas en `screenshots/`. HTML validado.
